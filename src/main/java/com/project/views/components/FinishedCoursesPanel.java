...
public class FinishedCoursesPanel extends VerticalLayout {

    private H2 finishedCoursesHeader;
    private Grid<Map<Integer, String>> coursesGrid;
    private PageNavigationComponent pageNavigationComponent;
    public static final int PAGE_SIZE = 10;

    public FinishedCoursesPanel(UserControllerInterface controllerInterface, BeforeEnterEvent beforeEnterEvent) {

        KeycloakAuthenticationToken authentication = (KeycloakAuthenticationToken)  SecurityContextHolder.getContext().getAuthentication();
        KeycloakPrincipal principal = ((KeycloakPrincipal) authentication.getPrincipal());
        String username = principal.getKeycloakSecurityContext().getToken().getPreferredUsername();

        finishedCoursesHeader = new H2("Finished courses");

        Location location = beforeEnterEvent.getLocation();
        QueryParameters queryParameters = location.getQueryParameters();

        int currentPage = 1;
        int pagesCount = (controllerInterface.getPassedUserCoursesCount(username).getBody() - 1)/LatestCoursesPanel.PAGE_SIZE + 1;
        ...
        List<Obj> courses = controllerInterface.getPassedUserCourses(username, currentPage,PAGE_SIZE).getBody();
        ...
        this.add(finishedCoursesHeader, coursesGrid, pageNavigationComponent);
    }
